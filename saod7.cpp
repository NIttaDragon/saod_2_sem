...
int main()
{
  ...
  for(int i = 0; i<b; i++) //заполнение матрицы координат
  {
    ...
    cout<<"x1=";  cin>>g;  otr[i][0] = g;
    cout<<"y1=";  cin>>g;  otr[i][1] = g;
    cout<<"x2=";  cin>>g;  otr[i][2] = g;
    cout<<"y1=";  cin>>g;  otr[i][3] = g;
  }
  ...
  for(int i = 0; i<b; i++)
  {
    ...
    if(otr[i][1]==otr[i][3])  a[i]=0;
    else  a[i]=(otr[i][3]-otr[i][1])/(otr[i][2]-otr[i][0]);
  }
  ...
  for(int i = 0; i<b-1; i++)
    for(int j =i+1; j<b;j++)
    {
      if(a[i]==a[j])  cout<<"Отрезки"<<i<<"и"<<j<<"параллельны \n";
      else
      {
